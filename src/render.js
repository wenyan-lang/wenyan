...
function render(fname,txt,{plotResult=false}={}){
	...
	function makeBorder(){
		...
		for (var i = 0; i < fname.length; i++){
			svg += `<text x="${(side?0:(W2-X1))+X1*0.1}" y="${vo+30+i*X1*1.5}" font-size="${X1*0.8}" font-family="${FONT}" fill="${BLACK}">${fname[i]}</text>`

		}
		var hpg = num2hanzi(pageno+1);
		for (var i = 0; i < hpg.length; i++){
			svg += `<text x="${(side?0:(W2-X1))+X1*0.1}" y="${uo+30+i*X1*1.5}" font-size="${X1*0.8}" font-family="${FONT}" fill="${BLACK}">${hpg[i]}</text>`
		}
	}
	...
	function newSvg(){
		svg = `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="${W2}" height="${H}" viewBox="${-MA} ${-MA} ${W2+MA*2} ${H+MA*2}">`;
		if (MA){
			svg += `<rect x="${-MA}" y="${-MA}" width="${W2+MA*2}" height="${H+MA*2}" stroke="none" fill="white"></rect>`
		}
	...
	function resultPlotter(){
		...
		for (var i = 0; i < out.length; i++){
			if (out[i] == "\n"){
				...
				svg += `<text x="${x}" y="${y}" font-size="${CS/3}" font-family="${FONT}" fill="rgba(0,0,0,0.7)">${out[i]}</text>`
				y+=CH/2.5;
				if (y >= Y1){
					y=Y0+CH;
					x-=CW/3.7;
	...
	for (var i = 0; i < txt.length; i++){
		...
		if (txt[i] == "\n"){
			...
			if (!iden && txt[i] == "批" && txt[i+1] == "曰"){
					...
					if (tokens[j][2]>i+1){
						...
						for (var k = 0; k < cmt.length; k++){
							if (cmt[k]=="。"){
								...
								svg += `<text x="${kx}" y="${ky}" font-size="${CS/2.2}" font-family="${FONT}" fill="${BOOK_COLORS.comment}">${cmt[k]}</text>`
								ky += CH/1.5;
								if (ky>Y0-PCB){
									ky = PCT+CH/2;
									kx-=CW/2;
				...
				svg += `<circle cx="${x+CW-3}" cy="${y-CH+PSH}" r="${CH/7}" stroke="${RED}" fill="none" stroke-width="1" wy-data="${txt[i]}"></circle>`
			}else if (txt[i] == "「"){
				iden = true;
				if (y > Y1){
					nextLine();
				...
				var h = CH/4
				svg += `<path d="M${x} ${y-CH/2+h/2+PSH} L${x+CW/5} ${y-CH/2-h/2+PSH} L${x+4*CW/5} ${y-CH/2-h/2+PSH} L${x+CW} ${y-CH/2+h/2+PSH}" stroke="${BLACK}" fill="none" stroke-width="0.5" wy-data="${txt[i]}"></path>`
				...
				var h = CH/4
				svg += `<path d="M${x} ${y-CH/2-h+PSH} L${x+CW/5} ${y-CH/2+PSH} L${x+4*CW/5} ${y-CH/2+PSH} L${x+CW} ${y-CH/2-h+PSH}" stroke="${BLACK}" fill="none" stroke-width="0.5" wy-data="${txt[i]}"></path>`
				...
				svg += `<text x="${x+CO}" y="${y}" font-size="${CS}" font-family="${FONT}" fill="${BOOK_COLORS[sm[i]]}" wy-data="${txt[i]}">${txt[i]}</text>`
				y += CH;
			}
